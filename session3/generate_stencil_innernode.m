...
dTdx_s= (1/S_s) * (dy_Sw_Se*T_S + dy_Se_e*T_se + dy_e_w*T_P + dy_w_Sw*T_sw);
dTdy_s= (-1/S_s) * (dx_Sw_Se*T_S + dx_Se_e*T_se + dx_e_w*T_P + dx_w_Sw*T_sw);

dTdx_e= (1/S_e) * (dy_s_sE*T_se + dy_sE_nE*T_E + dy_nE_n*T_ne + dy_n_s*T_P);
dTdy_e= (-1/S_e) * (dx_s_sE*T_se + dx_sE_nE*T_E + dx_nE_n*T_ne + dx_n_s*T_P);

dTdx_n= (1/S_n) * (dy_w_e*T_P + dy_e_Ne*T_ne + dy_Ne_Nw*T_N + dy_Nw_w*T_nw);
dTdy_n= (-1/S_n) * (dx_w_e*T_P + dx_e_Ne*T_ne + dx_Ne_Nw*T_N + dx_Nw_w*T_nw);

dTdx_w= (1/S_w) * (dy_sW_s*T_sw + dy_s_n*T_P + dy_n_nW*T_nw + dy_nW_sW*T_W);
dTdy_w= (-1/S_w) * (dx_sW_s*T_sw + dx_s_n*T_P + dx_n_nW*T_nw + dx_nW_sW*T_W);


% Build hole stecil acounting for quadratic lambda like in Helmholtz 

 DDT= (1/S_P) * (dy_sw_se*dTdx_s - dx_sw_se*dTdy_s + dy_se_ne*dTdx_e - dx_se_ne*dTdy_e ...
     + dy_ne_nw*dTdx_n - dx_ne_nw*dTdy_n + dy_nw_sw*dTdx_w - dy_nw_sw*dTdy_w);
