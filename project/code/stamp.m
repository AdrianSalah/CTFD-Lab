...
function [stecil,b] = stamp(i, j, X, Y, TD, alpha, Tinf, boundary, timestep, phase_vel, dTdt_p, IndexNorthBoundary)
...
%    timestep  currenttimestep for timedependent dirichlet BC
%    phase_vel phase velocity of wave
%    dTdt_p    finite difference approx. of timederivative at point p
...
if i==n
    nodePosition = 'South';
elseif sum(IndexNorthBoundary == index(i,j)) == 1
    nodePosition = 'North';
end
...
switch nodePosition
    ...
    case 'East'
        ...
        if strcmp(boundary.east, 'Open')
            ...
            b = (dTdt_p*dl_s_n)/(S_omega*phase_vel);
    ...
    case 'EastSouth'
        ...
        if strcmp(boundary.south, 'Neumann')
            ...
            elseif strcmp(boundary.east, 'Robin')
                ...
                b = -alpha*dl_P_n*Tinf/S_etaomega;
...
%         end
         ...
         % if south & east are dirichlet bc, interpolate values in corner
         elseif strcmp(boundary.south, 'Dirichlet')
    ...
    case 'EastNorth'
        ...
        if strcmp(boundary.north, 'Neumann') || strcmp(boundary.north, 'Robin')
            ...
            elseif strcmp(boundary.east, 'Robin')
                ...
                % P
                stecil(index(i, j)) = D0 - alpha*(dl_s_P+dl_P_w)/S_sigmaomega;
                
                b = -alpha*(dl_s_P+dl_P_w)*Tinf/S_sigmaomega;
        ...
        % if north & east are dirichlet bc, interpolate values in corner
        elseif strcmp(boundary.north, 'Dirichlet')
                if strcmp(boundary.east, 'Dirichlet')
                    stecil(index(i, j)) = 1;
                    % interpolate boundary values in corner
    ...
    case 'WestSouth'
        ...
        if strcmp(boundary.south, 'Neumann')
            ...
            elseif strcmp(boundary.west, 'Robin')
                ...
                % P
                stecil(index(i, j)) = D0 - alpha*dl_n_P/S_etaepsilon;
                
                b = -alpha*dl_n_P*Tinf/S_etaepsilon;
        ...
        % NOTE: currently no interpolation here

        % if south & west are dirichlet bc, interpolate values in corner
%         elseif strcmp(boundary.south, 'Dirichlet')
    ...
    case 'WestNorth'
        ...
        if strcmp(boundary.north, 'Neumann') || strcmp(boundary.north, 'Robin')
            ...
            elseif strcmp(boundary.west, 'Robin')
                ...
                % P
                stecil(index(i, j)) = D0 - alpha*(dl_P_s+dl_e_P)/S_sigmaepsilon;
                
                b = -alpha*T_inf*(dl_P_s+dl_e_P)/S_sigmaepsilon;
...
%                 b=0;
        ...
        elseif strcmp(boundary.north, 'Dirichlet')
            ...
            error ('no suitable boundary condition for northwest point');
        end
        
        % NOTE: currently no interpolation here

        % if north & west are dirichlet bc, interpolate values in corner
%         elseif strcmp(boundary.north, 'Dirichlet')
%                 if strcmp(boundary.west, 'Dirichlet')
%                     stecil(index(i, j)) = 1;
%                     % interpolate boundary values in corner
