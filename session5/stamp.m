function [stecil,b] = stamp(i, j, X, Y, TD, alpha, Tinf, boundary, betaeast, betanorth, betawest,k)
%stecil calculate the linear equation for node (i, j)
%
%  input:
%    betaeast  heat flux eastern boundary  
%    betanorth heat flux northern boundary 
%    betaeast  heat flux western boundary 
...
% Idea: Let all combinations of boundary conditions be possible, except:
% heat flux != 0 in south, Robin in South
% average overlapping Dirichlet conditions in corners
...
switch nodePosition
    ...
    case 'North'
        ...
        if strcmp(boundary.north, 'Neumann')
            ...
            b = betanorth*dl_w_e/S_sigma;
            
        elseif strcmp(boundary.north, 'Robin')
            b = -alpha*dl_w_e*Tinf/(k*S_sigma);
            stecil (index(i,j)) = stecil (index(i,j))-alpha*dl_w_e/(k*S_sigma);
    ...
    case 'East'
        ...
        if strcmp(boundary.east, 'Neumann')  
            b = betaeast*dl_s_n/S_omega;
        elseif strcmp(boundary.east, 'Robin')  
            b = -alpha*dl_s_n*Tinf/(k*S_omega);
             stecil (index(i,j)) = stecil (index(i,j))-alpha*dl_s_n/(k*S_omega);
    ...
    case 'West'
        ...
        if strcmp(boundary.west, 'Robin') || strcmp(boundary.west, 'Neumann')
            ...
            %differ between Neumann and Robin
          
        end  
        if strcmp(boundary.west, 'Neumann')  
            b = betawest*dl_s_n/S_epsilon;
        elseif strcmp(boundary.west, 'Robin')  
            b = -alpha*dl_s_n*Tinf/(k*S_epsilon);
             stecil (index(i,j)) = stecil (index(i,j))-alpha*dl_s_n/(k*S_epsilon);
    ...
    case 'EastSouth'
        ...
        if strcmp(boundary.south, 'Neumann')
            ...
            if strcmp(boundary.east, 'Neumann') || strcmp(boundary.east, 'Robin') % for east Dirichlet B.C. not recommended to calculate
                ...
                dl_P_w = norm ([dx_w_P; dy_w_P]); %%added
            ...
            if strcmp(boundary.east, 'Neumann')
                % P
                stecil(index(i, j)) = D0;
                b = (1/S_etaw)*(0*dl_P_w +betaeast*dl_P_n) ; 
                
            elseif strcmp(boundary.east, 'Robin')
                % P
                stecil(index(i, j)) = D0;   
                b = (0/S_etaw)*dl_P_w -alpha*Tinf*dl_P_n/(k*S_etaw);
                stecil (index(i,j)) = stecil (index(i,j))-alpha*dl_P_n/(k*S_etaw);
        ...
        elseif strcmp(boundary.south, 'Dirichlet')
            ...
            stecil(index(i, j)) = 1;
            % interpolate boundary values in corner
            b = (TD.south+TD.east)/2;
            else
            stecil(index(i, j)) = 1;
            b = TD.south;
            end
        else
             error ('no suitable boundary condition for southeast point');
    ...
    case 'EastNorth'
        ...
        if strcmp(boundary.north, 'Neumann') || strcmp(boundary.north, 'Robin') %% check if either north or east boundary has Dirichlet conditions
            ...
            if strcmp(boundary.east, 'Neumann') 
                switch (boundary.north)
                    case 'Neumann'
                         ...
                         b = (1/S_sigmaomega)*(betaeast*dl_s_P +betanorth*dl_P_w) ; 
                         
                    case 'Robin'
                         stecil(index(i, j)) = D0 ;
                         b = (1/S_sigmaomega)*betaeast*dl_s_P -alpha*dl_P_w*Tinf/(k*S_sigmaomega);
                         stecil (index(i,j)) = stecil (index(i,j))-alpha*dl_P_w/(k*S_sigmaomega);
                end
            ...
            elseif strcmp(boundary.east, 'Robin')
                ...
                % P
                        stecil(index(i, j)) = D0 ;
                        b = (betanorth/S_sigmaomega)*dl_P_w -alpha*Tinf*dl_s_P/(k*S_sigmaomega);
                        stecil (index(i,j)) = stecil (index(i,j))-alpha*dl_s_P/(k*S_sigmaomega);
                    case 'Robin'
                        
                        stecil(index(i, j)) = D0 ;
                        b = -alpha*Tinf*dl_P_w/(k*S_sigmaomega)-alpha*Tinf*dl_s_P/(k*S_sigmaomega);
                        stecil (index(i,j)) = stecil (index(i,j))-alpha*(dl_s_P+dl_P_w)/(k*S_sigmaomega);
                end
            elseif strcmp(boundary.east, 'Dirichlet')
    ...
    case 'WestSouth'
        ...
        if strcmp(boundary.south, 'Neumann')
            ...
            if strcmp(boundary.west, 'Neumann')
                % P
                stecil(index(i, j)) = D0;
                b = (1/S_etaepsilon)*(0 +betawest*dl_n_P) ; 
            ...
            elseif strcmp(boundary.west, 'Robin')
               ...
               b = 0-alpha*Tinf*dl_n_P/(k*S_etaepsilon);
             stecil (index(i,j)) = stecil (index(i,j))-alpha*dl_n_P/(k*S_etaepsilon);
                
            elseif strcmp(boundary.west, 'Dirichlet') %% bouth Dirichlet ==> make south Dirichlet
    ...
    case 'WestNorth'
        ...
        if strcmp(boundary.north, 'Neumann') || strcmp(boundary.north, 'Robin')
                ...
                switch (boundary.north)
                    case 'Neumann'
                        stecil(index(i, j)) = D0;
                         ...
                         b = (1/S_sigmaepsilon)*(betawest*dl_P_s +betanorth*dl_e_P) ; 
                         
                    case 'Robin'
                         stecil(index(i, j)) = D0 ;
                 b = (1/S_sigmaepsilon)*betawest*dl_P_s -alpha*dl_e_P*Tinf/(k*S_sigmaepsilon);
             stecil (index(i,j)) = stecil (index(i,j))-alpha*dl_e_P/(k*S_sigmaepsilon);
                end
            ...
            elseif strcmp(boundary.west, 'Robin')
                ...
                % P
                stecil(index(i, j)) = D0 ;
                 b = (betanorth/S_sigmaepsilon)*dl_e_P -alpha*Tinf*dl_P_s/(k*S_sigmaepsilon);
             stecil (index(i,j)) = stecil (index(i,j))-alpha*dl_P_s/(k*S_sigmaepsilon);
                    case 'Robin'
                        
                        stecil(index(i, j)) = D0 ;
                 b = -alpha*Tinf*dl_e_P/(k*S_sigmaepsilon)-alpha*Tinf*dl_P_s/(k*S_sigmaepsilon);
             stecil (index(i,j)) = stecil (index(i,j))-alpha*(dl_P_s+dl_e_P)/(k*S_sigmaepsilon);
                end
            elseif strcmp(boundary.west, 'Dirichlet') %% north over east => bouth Dirichlet, make north Dirichlet
