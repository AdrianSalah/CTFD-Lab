...
T_nomega = (0.75/2)*T_N + (0.75/2)*T_P + (0.25/2)*T_NW + (0.25/2)*T_W;
T_somega = (0.75/2)*T_S + (0.75/2)*T_P + (0.25/2)*T_SW + (0.25/2)*T_W;

% Gradients (Greens theorem)

dTdx_somega= (1/S_somega)*(dy_S_P*T_s+dy_P_w*T_omega+dy_w_Sw*T_sw+dy_Sw_S*T_Somega);
dTdy_somega= -(1/S_somega)*(dx_S_P*T_s+dx_P_w*T_omega+dx_w_Sw*T_sw+dx_Sw_S*T_Somega);

dTdx_nomega= (1/S_nomega)*(dy_w_P*T_omega+dy_P_N*T_n+dy_N_Nw*T_Nomega+dy_Nw_w*T_nomega);
dTdy_nomega= -(1/S_nomega)*(dx_w_P*T_omega+dx_P_N*T_n+dx_N_Nw*T_Nomega+dx_Nw_w*T_nomega);

dTdx_w= (1/S_w)*(dy_s_n*T_P+dy_n_nW*T_nw+dy_nW_sW*T_W+dy_sW_s*T_sw);
dTdy_w= -(1/S_w)*(dx_s_n*T_P+dx_n_nW*T_nw+dx_nW_sW*T_W+dx_sW_s*T_sw);

% Build hole stecil acounting for quadratic lambda like in Helmholtz 

  DDT=(1/S_omega)*(dy_sw_s*dTdx_somega -dx_sw_s*dTdy_somega +...
