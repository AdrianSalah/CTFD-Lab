function VisTemperature(formfunction, X, Y, T_vec, l, str)
...
for i = 0:size(T_vec,2)-1
surf(X, Y, reshape(T_vec(:,i+1),size(X,1),size(X,2)));
...
hold on
plot([0 linspace(0, l, 50) l], [0 -formfunction(linspace(0, 1, 50)) 0])
contour(X,Y,reshape(T_vec(:,i+1),size(X,1),size(X,2)))
contour(X,-Y,reshape(T_vec(:,i+1),size(X,1),size(X,2)))
% contour(X,-Y,T)
...
hold on
plot([0 linspace(0, l, 50) l], [0 -formfunction(linspace(0, 1, 50)) 0])
pcolor(X,Y,reshape(T_vec(:,i+1),size(X,1),size(X,2)))
shading interp
pcolor(X,-Y,reshape(T_vec(:,i+1),size(X,1),size(X,2)))
shading interp
% pcolor(X,-Y,T)
